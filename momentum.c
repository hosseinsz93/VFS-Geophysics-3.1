...
void Pressure_Gradient(UserCtx *user, Vec dP)
{
  ...
  /* First we calculate the flux on cell surfaces. Stored on the upper integer
	...
	DAGlobalToLocalBegin(user->da, user->P, INSERT_VALUES, user->lP); //Hossein tempoarily made it comment
...
//	for wave_momentum_source
	...
	if(wave_momentum_source || wave_sponge_layer) DAVecGetArray(fda, user->lWAVE_fp, &WAVE_fp);	
// end (Toni)
...
*/
	...
	// add (Toni)
	//for wave_momentum_source
	if(wave_momentum_source || wave_sponge_layer) DAVecRestoreArray(fda, user->lWAVE_fp, &WAVE_fp);	 
