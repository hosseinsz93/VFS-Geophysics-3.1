...
/*   user->st = 1.; */
 ...
 user->Ri = 0.0; 
  user->cfl=0.01;
  user->vnn=0.01;
if(density_current) {PetscOptionsGetReal(PETSC_NULL, "-richardson", &user->Ri, PETSC_NULL);}
  ...
  PetscOptionsGetReal(PETSC_NULL, "-dts", &user->dts, PETSC_NULL);
  ...
  PetscOptionsGetReal(PETSC_NULL, "-rho_fluid", &user->rho_fluid, PETSC_NULL); //KFlora used in rotor_model.c and fe_transfer.c so copied over
...
PetscErrorCode Read_Rotate_FSI_Parameter_Input(FSISetup *fsimov)
 {
	...
	FILE *f=fopen(str, "r");
        ...
        if (!f) {
             SETERRQ(1, "Cannot open fsi_rot.dat file");
