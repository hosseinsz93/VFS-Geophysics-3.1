...
PetscReal InletSyntheticTurbulence(PetscReal r, UserCtx *user)   //Open Channel Inlet Profile: generated turbulence
{
  	PetscReal base_velocity = InletInterpolation(r, user);  // Base velocity profile
    PetscReal turb_x, turb_y, turb_z;

    // Call GenerateTurb() to get synthetic turbulence
    //GenerateTurb(&turb_x, &turb_y, &turb_z);

    // Superimpose synthetic turbulence onto base velocity
    return base_velocity + turb_x;
}
