...
PetscErrorCode Do_averaging(UserCtx *user)
{
	...
	DAVecGetArray(user->da, user->lP, &p);
        ...
        if(sandwave){
	DAVecGetArray(user->da, user->lUstar, &ustar);
	DAVecGetArray(user->da, user->lUstar_sum, &ustar_sum);
	DAVecGetArray(user->da, user->lUstar_, &ustar_);
	}

        if(conv_diff){
	DAVecGetArray(user->da, user->Conc, &conc);
	DAVecGetArray(user->da, user->Conc_sum, &conc_sum);
	}

        if(levelset){
	DAVecGetArray(user->da, user->lLevelset, &level);
	DAVecGetArray(user->da, user->Level_sum, &level_sum);
	DAVecGetArray(user->da, user->Level_square_sum, &level2sum);
	}
	...
	DAVecRestoreArray(user->da, user->Conc_sum, &conc_sum);
	}
        
        if(levelset) {
	DAVecRestoreArray(user->da, user->lLevelset, &level);
	DAVecRestoreArray(user->da, user->Level_sum, &level_sum);
	DAVecRestoreArray(user->da, user->Level_square_sum, &level2sum);
	}

        if(sandwave) {
	DAVecRestoreArray(user->da, user->lUstar, &ustar);
	DAVecRestoreArray(user->da, user->lUstar_sum, &ustar_sum);
	DAVecRestoreArray(user->da, user->lUstar_, &ustar_);
	}
